    ...
    <div>
        <div class="row">
            <div class="col-md-9 col-sm-12">
                ...
                <input id="infoSincronizar" name="info" v-model="url" class="form-control" placeholder="Digite...">
            </div>

            <div class="col-md-3 col-sm-12">
                <button type="button" class="btn btn-primary btn-block btn-admin btn-admin-filter" title="Filtrar" @click="getJogos(url)">
        ...
        <div class="row" v-if="jogos.length > 0">
            <div class="col-sm-12">
                ...
                <div v-for="(jogo, key) in jogos">
                    <span>{{ key + 1 }} - </span><strong>{{ jogo.horario }} - </strong><span>
        ...
        <div class="row" v-if="outros.length > 0">
            <div class="col-sm-12">
                ...
                <div v-for="(item, key) in outros">
                    <strong>{{ key + 1 }} - </strong><span>{{ item }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-2">
                <button type="button" class="btn btn-primary btn-admin-filter" @click="saveJogos()" v-if="jogos.length > 0">
                    Salvar Jogos
                </button>
            </div>
        </div>
...
<script>
    export default {
        ...
        methods: {
            getJogos(url) {
                this.$http.post('/api/jogo/getJogosGE', { url }).then((response) => {
                    if (response.data) {
                        this.message = "";
            ...
            saveJogos() {
                ...
                if (this.jogos.length > 0) {
                    ...
                    this.$http.post('/api/jogo/save', { jogos }).then((response) => {
