    ...
    <div v-else>
        ...
        <div class="row">
          <div class="col-md-3 col-sm-4 col-xs-12">
            <select
              ...
              @change="getPalpites(participanteId, campeonato.id, rodada)"
            >
              <option
                :key="key"
                v-for="(participante, key) in participantes"
                :value="participante.user.id"
              >{{ participante.user.name }}</option>
            </select>
          </div>
        </div>
      </div>
      <div class="col-sm-12 box">
        <div class="btn-group btn-rodada" role="group">
          <button
            ...
            :disabled="rodada < 2"
            @click="getPalpites(user.id, campeonato.id, rodada - 1)"
          >
            ...
            <div class="dropdown-menu dropdown-rodada">
              <div class="flex-container">
                <div class="flex-items" v-for="(n, key) in campeonato.qtd_rodadas" :key="key">
                  <a
                    @click="getPalpites(user.id, campeonato.id, n)"
      ...
      <div v-if="!dataLoading">
              ...
              <div class="td">
                ...
                <div class="placar">
                  ...
                  <div class="numbers">
                    <input
                      type="number"
                      :disabled="checkTime(jogo) == jogo.id || saveLoading"
                      min="0"
                    ...
                    <span
                      v-else
                    >{{ (!checkTime(jogo) && user.id !== participanteId) ? "x" : jogo.placar_casa }}</span>
                    x
                    <input
                      type="number"
                      :disabled="checkTime(jogo) == jogo.id || saveLoading"
                      ...
                      v-model="jogo.palpite.fora"
                      @blur="savePalpite(jogo)"
                    />

                    <span
                      v-else
                    >{{ (!checkTime(jogo) && user.id !== participanteId) ? "x" : jogo.placar_fora }}</span>
                  </div>

                  <strong
                    class="text-danger"
                    v-if="checkTime(jogo) == jogo.id && jogo.placar_casa === null && user.id === participanteId"
                  >Esgotado!</strong>
                </div>
              ...
              <div class="td" v-if="user.id === participanteId">
                <div v-show="(checkTime(jogo) != jogo.id) && !saveLoading">
                  <a
                    href
                    v-if="(jogo.placar_casa !== null || jogo.placar_fora !== null)"
                    @click.prevent="savePalpite(jogo, true)"
                  >
                    <i
        ...
        <div class="col-sm-12 box">
          ...
          <p>
            <i class="glyphicon glyphicon-stop exato"></i>
            - Placar Exato ({{ bolao.placar_exato }} pontos),
          </p>
          <p>
            <i class="glyphicon glyphicon-stop vencedor"></i>
            - Placar Vencedor ({{ bolao.placar_vencedor }} pontos).
          </p>
        </div>
      </div>
    </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <table class="table">
              <tbody>
                <tr v-for="(item, key) in comparacao" :key="key">
                  <td>{{ key + 1 }}</td>
                  <td>{{ item.name }}</td>
...
export default {
  ...
  methods: {
    ...
    getParticipantes(bolaoId) {
      this.dataLoading = true;
      this.$http
        .get(`/api/participante/getBolao/${bolaoId}`)
        .then((response) => {
    ...
    getPalpites(userId, campeonatoId, rodada) {
      ...
      if (userId > 0 && campeonatoId > 0) {
        this.$http
          .get(`/api/palpite/getPalpites/${userId}/${campeonatoId}/${rodada}`)
          .then((response) => {
    ...
    compararPalpites(jogo) {
      if (jogo.id > 0) {
        this.$http
          .get(`/api/palpite/compararPalpites/${jogo.id}`)
          .then((response) => {
