  ...
  <div>
      ...
      <div class="row">
        <div class="col-sm-4">
          <select
            class="form-control"
            v-model="bolaoId"
            @change="getParticipantesByBolao(bolaoId);"
          >
            <option value="0">Selecione o bolão...</option>
            <option v-for="bolao in bolaos" :key="bolao.id" :value="bolao.id">{{ bolao.nome }}</option>
          </select>
        </div>
      </div>
    </div>
    ...
    <div class="col-sm-12 box">
      ...
      <table class="table table-bordered mtb-20" v-if="participantes.length > 0">
        ...
        <tbody>
          <tr v-for="(participante, key) in participantes" :key="key">
...
export default {
  ...
  methods: {
    ...
    addParticipante(userId) {
      ...
      this.$http
        .post("/api/participante/create", data)
        .then((response) => {
          this.userId = 0;
          this.getParticipantesByBolao(this.bolaoId);
        })
    ...
    deleteParticipante(participanteId) {
      this.$http
        .post("/api/participante/delete/" + participanteId)
        .then((response) => {
          this.userId = 0;
          this.getParticipantesByBolao(this.bolaoId);
        })
