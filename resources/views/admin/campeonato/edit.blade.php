        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-3 sm:p-6 bg-white border-b border-gray-200">
                    <form id="form-edit" method="POST" action="{{ route('admin.campeonatos.update', $campeonato->id) }}">
                        @method('PUT')
                        @csrf
                            ...
                            @if($field['type'] === 'select')
                                <select name="{{ $field['name'] }}" class="vb-custom-input italic">
                                    @foreach(${$field['options']} as $option)
                                        <option value="{{ $option->id }}" {{ $option->id === $campeonato->{$field['name']} ? 'selected' : '' }}>
                                            {{ $option->nome }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <form id="form-delete" action="{{ route('admin.campeonatos.destroy', $campeonato->id) }}" method="POST" class="hidden">
                        @method('DELETE')
                        @csrf
                    </form>
                    ...
                    <div class="grid gap-4 mt-4">
                        ...
                        <a class="btn-secondary col-end-6" href="{{ route('admin.campeonatos.index') }}">Cancelar</a>
