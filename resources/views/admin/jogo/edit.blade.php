        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-3 sm:p-6 bg-white dark:bg-slate-800 dark:text-slate-300 border-b border-gray-200 last:border-b-0">
                    <form id="form-edit" method="POST" action="{{ route('admin.jogos.update', $jogo->id) }}">
                        @method('PUT')
                        @csrf
                        ...
                        <label class="block pb-4">
                            ...
                            @if($field['type'] === 'select')
                                <select name="{{ $field['name'] }}" class="vb-custom-input italic">
                                    @foreach(${$field['options']} as $option)
                                        <option value="{{ $option->id }}" {{ $option->id === $jogo->{$field['name']} ? 'selected' : '' }}>
                                            {{ $option->nome }}
                                        </option>
                                    @endforeach
                                </select>
                            @else
                                <input class="vb-custom-input italic"
                                       type="{{ $field['type'] }}"
                                       name="{{ $field['name'] }}"
                                       value="{{ $field['name'] === 'inicio' ? \Carbon\Carbon::parse($jogo->inicio)->format('Y-m-d H:i:s') : $jogo->{$field['name']} }}"
                    ...
                    <form id="form-delete" action="{{ route('admin.jogos.destroy', $jogo->id) }}" method="POST" class="hidden">
                        @method('DELETE')
                        @csrf
                    </form>
